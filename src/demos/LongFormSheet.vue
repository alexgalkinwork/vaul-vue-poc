...
const fields = ref(
  Array.from({ length: 12 }, (_, i) => ({ label: `Field ${i + 1}`, value: '' }))
)
      ...
      <DrawerContent class="fixed bottom-0 left-0 right-0 flex flex-col rounded-t-2xl bg-white max-h-[96dvh]">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4" data-vaul-no-drag>
          ...
          <div class="flex gap-3 pb-4">
            ...
            <button class="flex-1 rounded-lg bg-blue-600 px-4 py-3 text-white font-medium active:bg-blue-700" @click="open = false">Save All</button>
          </div>
        </div>
      </DrawerContent>
    </DrawerPortal>
